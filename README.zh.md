...
- 检查关键的插件 manifest、工作流 Skill 与 hook 文件，再调用稳定的 `codex plugin marketplace` 与 `codex plugin` 子命令
- 添加本仓库 marketplace 并安装 `oh-my-paper-codex`，不覆盖无关 marketplace 或插件
- 底层使用 `node` 执行，所以请先确保 `node` 和 `codex` 都在你的 `PATH` 上

插件安装期间请保留这个克隆目录：它是本地 marketplace 的来源。可运行 `node scripts/manage-codex-plugin.mjs status` 查看状态；拉取更新后重跑安装脚本即可升级，或运行对应卸载脚本。状态会分别报告 marketplace 可见性、插件安装状态、Skills 打包状态和 hook 信任状态。
